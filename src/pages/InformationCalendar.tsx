...
const InformationCalendar = () => {
    ...
    const loadData = async () => {
        ...
        const response = await axios.get<{ data: any[] }>(
            ...
            { params: { userId: 1, month: month, year: year } }
        );
                ...
                if (data.gnrl_rnk1_crsparea_rcptde && data.gnrl_rnk1_crsparea_endde) {
                    ...
                    while (start <= end) {
                        eventList.push({
                            ...
                            pblanc_no: data.pblanc_no,
                            house_manage_no: data.house_manage_no,
    ...
    return (
        <Container>
            ...
            <HeaderCalendarWrapper>
                <HeaderCalendar onDateChange={handleDateChange} onFilterChange={handleFilterChange} />
            </HeaderCalendarWrapper>
            <Content>
                <CalendarSection>
                    <Calendar
                    ...
                    {selectedDateEvents.length > 0 ? (
                        selectedDateEvents.map((event) => (
                            <Card
                                ...
                                pblanc_no={event.pblanc_no}
                                house_manage_no={event.house_manage_no}
                                house_secd={event.house_secd}
                                liked={event.isBookmarked}
                                userId={1}
                            />
                        ))
                    ) : (
                        <NoEventsText>선택한 날짜에 해당하는 청약 정보가 없습니다.</NoEventsText>
                    )}
                </EventCardsWrapper>
            </Content>
        </Container>
