...
const KakaoMap: React.FC = () => {
    ...
    const [selectedLocation, setSelectedLocation] = useState<Location>({ city: "default", district: "default", latitude: 37.4197626, longitude: 126.8829669 });
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (input.trim() === "") {
        ...
        const filtered = seoulLocations.filter((location) =>
          location.district.startsWith(input)
        );
        ...
        >
          <ul style={{ margin: 0, padding: "10px", listStyle: "none" }}>
            {filteredLocations.map((location, index) => (
              <li key={index} style={{ padding: "5px 0", cursor: "pointer" }} onClick={()=>{
